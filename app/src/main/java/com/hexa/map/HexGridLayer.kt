...
/** Identifiant de la couche de remplissage qui surligne la tuile courante. */
 ...
 * de chaque cellule (seule la tuile courante reçoit une teinte, les autres restent transparentes)
 * sous un contour de ligne tracé pour toutes les cellules. Ensuite, ne fait que **réinjecter les
 * données** dans la source existante — bien moins coûteux que de recréer les couches, pour rester
 * fluide quand le joueur change de cellule. Chaque cellule est un polygone fermé : le remplissage en
 * colore l'intérieur, la ligne en souligne le pourtour.
 *
 ...
 * teinte de la tuile courante, transparent pour les cellules normales (valeur par défaut). Les alphas
 * sont portés par les couleurs `rgba` de [MapConfig].
 */
