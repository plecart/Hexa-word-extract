 ...
 *
 * Le premier fix **prime** : tant que [firstFix] est `null`, on reste sur [StartupStage.LOADING] quel
 * que soit l'état du compte — on ne montre jamais la carte (donc jamais un centre arbitraire) ni la
 * pose de base avant la position. Une fois la position connue, la pose de base
 ...
 *
 * @param firstFix dernière position filtrée connue, ou `null` tant qu'aucun fix n'est arrivé.
 * @param playerState état observable du compte joueur.
 */
fun startupStage(firstFix: LatLng?, playerState: PlayerUiState): StartupStage = when {
    firstFix == null -> StartupStage.LOADING
